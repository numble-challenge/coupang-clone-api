...
const isNumberString = (input: unknown) => {
  return input.toString() === Number(input).toString();
};
...
export default cors(async function handler(
  ...
  if (!req.query.offset || !req.query.limit) {
    res
      .status(400)
      .send("query parameter offset, limit이 제공되지 않았습니다.");
  }
  if (!isNumberString(req.query.offset) || !isNumberString(req.query.limit)) {
    res
      .status(400)
      .send("query parameter offset, limit은 숫자만 입력 가능합니다.");
  }
  if (!req.query.sorter) {
    res.status(400).send("query parameter sorter가 제공되지 않았습니다.");
  }
  if (!sorters.includes(req.query.sorter.toString())) {
    res
      .status(400)
      .send(
        `query parameter sorter는 "bestAsc", "salePriceAsc", "salePriceDesc", "saleCountDesc", "latestAsc"만 입력 가능합니다.`
      );
  }
  ...
  const sorter = req.query.sorter as Sorter;

  res.status(200).json(
    PRODUCTS.sort((a, b) => {
      if (sorter === "bestAsc") {
        return b.rating - a.rating;
      } else if (sorter === "salePriceAsc") {
        return a.salePrice - b.salePrice;
      } else if (sorter === "salePriceDesc") {
        return b.salePrice - a.salePrice;
      } else if (sorter === "saleCountDesc") {
